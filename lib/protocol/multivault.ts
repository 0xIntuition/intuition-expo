...
export class Multivault {
  ...
  constructor(
    private client: {
      publicClient: PublicClient<Transport, Chain, ParseAccount<Address>, RpcSchema>
      walletClient: WalletClient<Transport, Chain, ParseAccount<Address>, RpcSchema>
    },
    address?: Address,
  ) {
  ...
  public async waitForAtomCreatedTransaction(hash: `0x${string}`): Promise<{
    ...
    const { logs, status } = await this.client.publicClient.waitForTransactionReceipt(
      { hash },
    )
